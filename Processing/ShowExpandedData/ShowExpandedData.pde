...
//        println(d_range);
        ...
        for(int i=1;i<=16;i++)
        {
          ...
          y_val = int(pieces[5]);
          if(i<9) a_range = 1000;
          if(((i-1)%8)<4) b_range = 1000;
          if(((i-1)%4)<2) c_range = 1000;
          if((i%2)==1) d_range = 1000; 
          //println("A:"+a_range+" B:"+b_range+" C:"+c_range+" D:"+d_range);
          //println("No:"+i);
          calculate();
          
          values1 += "," + a_range + "," + b_range + "," + c_range + "," + d_range;
          
          values1 += "," + x_actual + "," + y_actual;
...
//          point(absXPoint(ind_to_point(x_ind_val)), absYPoint(ind_to_point(y_ind_val)));
          ...
          values1 += "," + x_meth_1 + "," + y_meth_1 + "," + x_meth_2 + "," + y_meth_2 + "," + x_val + "," + y_val + "," + i;
          output.println(values1);
        }
...
void wrapup() {
      ...
      output1.println("R,C,A_exact,B_exact,C_exact,D_exact,A_meas,B_meas,C_meas,D_meas,Exact_x,Exact_y,M1_x,M1_y,M2_x,M2_y,H_x,H_y,Mode");
