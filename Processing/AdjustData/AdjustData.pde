...
void draw() 
{
      ...
      for (int m=0;m < lines.length;m++)
      {
        ...
        {  
          ...
          for(int n=0;n<6;n++) 
          {
            ...
            data[m][n] = int(pieces[n]);
            if((completed[x_ind_val-1][y_ind_val-1] == 1)&&(n<4))
            {
              ...
              else if(data[m][n]>1000)
              {
                data[m][n] = -1*(65536 - data[m][n]) + constant;
              }
              else
              {
                data[m][n] = data[m][n] + constant;
              }
  ...
  //        point(absXPoint(x_val),absYPoint(y_val));
      ...
      completed[x_ind_val-1][y_ind_val-1]=0;
      int ix=x_ind_val -1;
      int jx=y_ind_val - 1;
      if(((ix%2)==0) && ((jx%2)==1))
      grid[ix][jx].fillColor = 'g';
      else
      grid[ix][jx].fillColor = 'w';
